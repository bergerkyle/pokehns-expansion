...
#else
    { COMPOUND_STRING("Check wall clock…"), DebugAction_ExecuteScript, NewBarkTown_PlayersHouse_2F_EventScript_CheckWallClock },
    { COMPOUND_STRING("Set wall clock…"),   DebugAction_ExecuteScript, NewBarkTown_PlayersHouse_2F_EventScript_SetWallClock },
#endif
    { NULL }
};
...
// *******************************
// Actions Sound
static const u8 *const sBGMNames[SONGS_END - SONGS_START + 1];
...
static void DebugAction_Sound_MUS_SelectId(u8 taskId)
{
    if (JOY_NEW(DPAD_ANY))
    {
        ...
        StringCopy(gStringVar2, gText_DigitIndicator[gTasks[taskId].tDigit]);
        bgmName = sBGMNames[gTasks[taskId].tInput - MUS_LITTLEROOT_TEST];
        if (bgmName == NULL)
...
#define X(songId) [songId - SONGS_START] = sBGMName_##songId,
static const u8 *const sBGMNames[SONGS_END - SONGS_START + 1] =
{
SOUND_LIST_BGM
};
