...
static bool8 CanSelectedMonUseUniqueHM(u8 *slotPtr)
{
    ...
    enum FieldMove i = 0;
    while (i != FIELD_MOVE_HM_END)
    {
        if (CanMonUseFieldMove(&gPlayerParty[*slotPtr], gFieldMoveInfo[i].moveID))
        {
            for (u32 j = 0; j < PARTY_SIZE; j++)
            {
                if (j != *slotPtr && CanMonUseFieldMove(&gPlayerParty[j], gFieldMoveInfo[i].moveID))
                    break;
            }
            return TRUE;
        }
...
static void CursorCb_FieldMove(u8 taskId)
{
    ...
    {
        // if (!CanPlayerUseFieldMove(gFieldMoveInfo[fieldMove].moveID)) - HnS / 1.13.0
...
// EMERALD
bool32 UNUSED SetUpFieldMove_Dive(void)
{
...
bool8 PlayerHasMove(u16 move)
{
    ...
    case MOVE_WATERFALL:
        item = ITEM_HM07;
        break;
    case MOVE_WHIRLPOOL: // HnS; was Dive
        item = ITEM_HM08;
        break;
    default:
        return FALSE;
